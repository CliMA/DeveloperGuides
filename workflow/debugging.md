...
A GPU-only `DomainError` that a CPU run steps through could be a Float32 rounding artifact: a quantity rounds to the out-of-domain side on GPU and to the valid side on CPU. The fix could be a floor or clamp at the quantity, ideally where it is computed.
